...
However, for the general case, we can only show that h' is admissible, which follows from the aforementioned inequality h'(x and y, z) ≤ d'(x and y, z) + 2 h'(z) for all z satisfying h'(z) = 0.